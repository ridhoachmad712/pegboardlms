...
<div class="card card-md"><div class="card-body">
    <h1 class="h2 mb-3">Ganti password sementara</h1>
    <p class="text-secondary">Admin telah mereset password Anda. Buat password pribadi sebelum melanjutkan ke aplikasi.</p>
    <form method="POST" action="{{ route('lecturer.password.update') }}">
        @csrf @method('PUT')
        <div class="mb-3">
            <label for="current-password" class="form-label">Password sementara dari admin</label>
        ...
        <div class="mb-3">
            ...
            <div id="password-help" class="form-hint">Minimal 12 karakter. Gunakan password yang berbeda dari password sementara.</div>
            @error('password')<div id="password-error" class="invalid-feedback">{{ $message }}</div>@enderror
        </div>
        <div class="mb-3">
            <label for="password-confirmation" class="form-label">Ulangi password baru</label>
